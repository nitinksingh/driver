...
def score_pathways(path_df, other_df, data_type, opt=''):
    ...
    """
    score_df = pd.DataFrame(index=path_df.columns, columns=other_df.columns)
    for p in path_df.columns:
        p_df = other_df.loc[path_df[p].dropna()]
...
def scorer(df, data_type, opt):
    ...
    if data_type == 'mut':
        if opt == 'silent':
            silent = df.replace([-1, 0], [np.nan, np.nan]).count()
            ret_s = silent/df.shape[0]
        elif opt == 'nsilent':
            nsilent = df.replace([1, 0], [np.nan, np.nan]).count()
